...
const CACHE_TTL = 300; // 5 minuta u sekundama

// Pomoćna funkcija za Caching
const withCache = async (cacheKey, fetchFunction) => {
  ...
  logger.info(`🗄️ Cache MISS: ${cacheKey}. Fetching from DB...`);
...
const getDailyRevenue = async (restaurantId, date = new Date()) => {
  ...
  const cacheKey = `revenue:daily:${restaurantId}:${start.toISOString().split('T')[0]}`;
...
const getMonthlyRevenue = async (restaurantId, month, year) => {
  ...
  const cacheKey = `revenue:monthly:${restaurantId}:${year}-${month}`;
...
const getDashboardSummary = async (restaurantId) => {
  ...
  const cacheKey = `dashboard:summary:${restaurantId}:${dateStr}`;
